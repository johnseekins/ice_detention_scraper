...
class ExternalDataEnricher:
    ...
    def enrich_facility_data(self, facilities_data):
        print("Starting data enrichment with external sources...")
        enriched_data = []
        total = len(facilities_data)

        for i, facility in enumerate(facilities_data):
            print(f"Processing facility {i+1}/{total}: {facility['name'][:50]}...")
    ...
    def _search_wikipedia(self, facility_name):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                facility_indicators = [
                    'detention', 'prison', 'jail', 'correctional', 'penitentiary',
                    'facility', 'center', 'complex', 'institution', 'processing'
                ]
            ...
            for query_attempt, search_query in enumerate(search_queries):
                ...
                if data.get('query', {}).get('search'):
                    ...
                    for result in data['query']['search']:
                        ...
                        facility_terms = ['detention', 'prison', 'jail', 'correctional',
                                          'penitentiary', 'facility', 'center', 'complex', 'processing']
                        ...
                        detention_context = ['detention', 'prison', 'jail', 'correctional',
                                             'inmates', 'custody', 'incarceration', 'processing']
                        ...
                        if (any(term in title_lower for term in generic_indicators) and
                                not any(term in title_lower for term in facility_terms)):
                            relevance_score -= 3
                        ...
                        if relevance_score > 0:
                            ...
                            if verify_response.status_code == 200:
                                ...
                                debug_info[
                                    'search_query'] = f"{search_query} -> {page_title} (score: {relevance_score})"
            ...
            print(f"    Wikipedia search error for '{facility_name}': {e}")
            return None
        ...
        try:
            ...
            if data.get('search'):
                for result in data['search']:
                    ...
                    if any(term in description for term in ['prison', 'detention', 'correctional', 'jail', 'facility', 'processing']):
                        if self.debug_wikidata:
                            return {
            ...
            print(f" Wikidata search error for '{facility_name}': {e}")
            if self.debug_wikidata:
    ...
    def _search_openstreetmap(self, facility_name, address):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data:
                    for result in data:
                        ...
                        if (any(term in osm_type for term in ['prison', 'detention', 'correctional']) or
                                any(term in display_name for term in ['prison', 'detention', 'correctional', 'jail'])):
            ...
            print(f" OSM search error for '{facility_name}': {e}")
            if self.debug_osm:
                return {'url': None, 'title': f"ERROR: {str(e)}"}
            return None
    ...
    def _clean_facility_name(self, name):
        ...
        suffixes_to_remove = [
            'Detention Center', 'Processing Center', 'Correctional Center',
            'Correctional Facility', 'Detention Facility', 'Service Processing Center',
            'ICE Processing Center', 'Immigration Processing Center',
            'Adult Detention Facility', 'Contract Detention Facility',
            'Regional Detention Center', 'County Jail', 'County Detention Center',
            'Sheriff\'s Office', 'Justice Center', 'Safety Center', 'Jail Services',
            'Correctional Complex', 'Public Safety Complex'
        ]
